...
| C4 | String error | text | `rg -n "Effect\.fail\(['\"]" $EFFECT_FILES` |
...
| Tier | Rules | Tool |
|------|-------|------|
| text | C1, C2, C4, C7, W1, W2, W3, W4, W8 | `rg` — no file reading |
| text+confirm | C9, W9 | `rg` pre-filter → LLM confirms within 5-line window |
| ast | C3, C6, W6 | `ast-grep scan` — no file reading |
| semantic | C5, C8, C10, W5, W7 | LLM reads file — judgment required |

## Hybrid Rule Protocol

For rules marked **text+confirm** (C9, W9):
